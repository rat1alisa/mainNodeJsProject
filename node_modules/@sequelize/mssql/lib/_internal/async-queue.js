...
class AsyncQueue {
  ...
  close() {
    this.closed = true;
    this.rejectCurrent(
      new import_core.ConnectionError(
        new import_async_queue_error.AsyncQueueError("the connection was closed before this query could finish executing")
      )
    );
  }
    ...
    return new Promise((resolve, reject) => {
      this.previous = this.previous.then(async () => {
        ...
        if (this.closed) {
          return reject(
            new import_core.ConnectionError(
              new import_async_queue_error.AsyncQueueError("the connection was closed before this query could be executed")
            )
          );
        }
